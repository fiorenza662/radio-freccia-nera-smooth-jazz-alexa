...
async function play(h){const cover=await art();return h.responseBuilder.speak(TITLE+'.').addDirective({type:'AudioPlayer.Play',playBehavior:'REPLACE_ALL',audioItem:{stream:{url:STREAM,token:'rfn-smooth-jazz-'+Date.now(),offsetInMilliseconds:0},metadata:{title:TITLE,subtitle:SUBTITLE,art:{sources:[{url:cover}]},backgroundImage:{sources:[{url:cover}]}}}}).withShouldEndSession(true).getResponse();}
