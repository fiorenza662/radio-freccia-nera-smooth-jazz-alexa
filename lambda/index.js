...
function streamDirective(tokenPrefix,m){m=m||{title:'Radio Freccia Nera Smooth Jazz',subtitle:'Radio Freccia Nera',art:LOGO};const nonce=Date.now();return{type:'AudioPlayer.Play',playBehavior:'REPLACE_ALL',audioItem:{stream:{url:STREAM,token:tokenPrefix+'-'+nonce,offsetInMilliseconds:0},metadata:{title:m.title,subtitle:m.subtitle,art:{sources:[{url:m.art}]},backgroundImage:{sources:[{url:m.art}]}}}};}
async function play(h){const m=await meta();console.log('RFN LIVE META',m.title,'|',m.subtitle);return h.responseBuilder.speak('Radio Freccia Nera Smooth Jazz.').addDirective(streamDirective('rfn-smooth-jazz',m)).withShouldEndSession(true).getResponse();}
